...
func processConfirmSig(
	...
	position types.PlasmaPosition, addr common.Address, sigs [2]types.Signature) (
	res sdk.Result) {
	...
	pubKey0, err0 := ethcrypto.SigToPub(confirmHash, sigs[0].Bytes())
	...
	if utils.ValidAddress(inputAddresses[1]) {
		pubKey1, err1 := ethcrypto.SigToPub(confirmHash, sigs[1].Bytes())
